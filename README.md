...
## Monitoring: localhost vs host.docker.internal

When DockerHQ runs inside a container, `localhost` in the Monitor tab refers to the **container's own network** — not your host machine. So `localhost:3500` will show as DOWN even though the app is running fine.

**Use `host.docker.internal` instead:**

| What to monitor | Host to use |
|---|---|
| DockerHQ itself | `host.docker.internal` |
| Any service mapped to a host port | `host.docker.internal` |
| Another container on the same Docker network | the container's service name (e.g. `postgres`) |

`host.docker.internal` works automatically on **Mac and Windows** with Docker Desktop.

On **Linux**, add this to `docker-compose.yml`:
